...
它目前用于确认课程规划；每节课自己的在线学习页与随机测验已在 `lessons/<课程-id>/` 中实现（内容处于 `awaiting-user-review`，尚待人工审校），做完一节后页面会给出「继续学习」入口，按课程顺序指向下一课的课件——最后一课换成课程收尾出口。视频始终由仓库外流程负责。后续交付范围与验收标准见 [`docs/roadmap.md`](./docs/roadmap.md)。
