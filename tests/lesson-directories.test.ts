...
test("S3-07 supplies a guided stack-and-queue lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s3-07");

  expect(manifest.faqTermIds).toContain("stack");
  expect(manifest.faqTermIds).toContain("queue");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S3-07"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s3-07-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
...
test("S3-08 supplies a guided dynamic-programming lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s3-08");

  expect(manifest.faqTermIds).toContain("dynamic-programming");
  expect(manifest.faqTermIds).toContain("dp-state");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S3-08"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s3-08-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("dp[i]");
  expect(source).toContain("dp[1] = 1");
  expect(source).toContain("dp[3] = 3");
});

test("S4-01 supplies a guided contest-statement lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-01");

  expect(manifest.faqTermIds).toContain("contest-program");
  expect(manifest.faqTermIds).toContain("data-range");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-01"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-01-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("请输入");
  expect(source).toContain("n <= 1000");
...
test("S4-03 supplies a guided simulation lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-03");

  expect(manifest.faqTermIds).toContain("state-table");
  expect(manifest.faqTermIds).toContain("event-order");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-03"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-03-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("+5 -3 -10");
  expect(source).toContain("5 → 2 → 跳过");
  expect(source).toContain("状态表");
});

test("S4-04 supplies a guided search-and-pruning lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-04");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-04"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-04-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("1、2、3");
  expect(source).toContain("target = 4");
  expect(source).toContain("搜索树");
});

test("S4-05 supplies a guided sorting-search-and-mapping lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-05");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-05"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-05-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("90 78 85 90 85 90");
  expect(source).toContain("78 85 85 90 90 90");
  expect(source).toContain("计数数组");
});

test("S4-06 supplies a guided DP-classic-model lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-06");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-06"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-06-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("3 7 4 6");
  expect(source).toContain("dp[3] = 13");
  expect(source).toContain("选或不选");
  expect(source).toContain("最优子结构");
  expect(source).toContain("状态压缩");
});

test("S4-07 supplies a guided graph-traversal lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-07");

  expect(manifest.faqTermIds).toContain("graph");
  expect(manifest.faqTermIds).toContain("adjacency-list");
  expect(manifest.faqTermIds).toContain("visited-marker");
  expect(manifest.faqTermIds).toContain("dfs");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-07"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-07-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("(0,1) (1,2) (3,4)");
  expect(source).toContain("连通块数 = 2");
  expect(source).toContain("邻接表");
  expect(source).toContain("访问标记");
});

test("S4-08 supplies a guided subtask-and-scoring-strategy lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s4-08");

  expect(manifest.faqTermIds).toContain("subtask");
  expect(manifest.faqTermIds).toContain("partial-score");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S4-08"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s4-08-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("子任务 1（n≤1000，30 分，暴力可过）");
  expect(source).toContain("保底方案");
  expect(source).toContain("先易后难");
...
test("S5-02 supplies a guided systematic-debug lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-02");

  expect(manifest.faqTermIds).toContain("systematic-debug");
  expect(manifest.faqTermIds).toContain("minimal-repro");
  expect(manifest.faqTermIds).toContain("debug-print");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-02"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-02-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("最小复现");
  expect(source).toContain("分段输出");
  expect(source).toContain("断言思路");
  expect(source).toContain("差分检查");
  expect(source).toContain("按证据");
});

test("S5-03 supplies a guided trap-review lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-03");

  expect(manifest.faqTermIds).toContain("overflow");
  expect(manifest.faqTermIds).toContain("long-long");
  expect(manifest.faqTermIds).toContain("array-init");
  expect(manifest.faqTermIds).toContain("out-of-bounds");
  expect(manifest.faqTermIds).toContain("precedence");
  expect(manifest.faqTermIds).toContain("equal-equal");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-03"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-03-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("溢出");
  expect(source).toContain("数组越界");
  expect(source).toContain("long long");
  expect(source).toContain("运算优先级");
  expect(source).toContain("== 比较相等");
});

test("S5-04 supplies a guided 80-percent-strategy lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-04");

  expect(manifest.faqTermIds).toContain("brute-force-baseline");
  expect(manifest.faqTermIds).toContain("subtask");
  expect(manifest.faqTermIds).toContain("special-case");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-04"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-04-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("暴力基线");
  expect(source).toContain("特殊情况");
  expect(source).toContain("复杂度升级");
  expect(source).toContain("伪优化");
  expect(source).toContain("差分检查");
});

test("S5-05 supplies a guided mock-exam lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-05");

  expect(manifest.faqTermIds).toContain("mock-exam");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-05"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-05-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("提交顺序");
  expect(source).toContain("自测表");
  expect(source).toContain("时间记录");
  expect(source).toContain("如实记录");
});

test("S5-06 supplies a guided error-classification lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-06");

  expect(manifest.faqTermIds).toContain("error-classification");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-06"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-06-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("读题错");
  expect(source).toContain("算法错");
  expect(source).toContain("实现错");
  expect(source).toContain("边界错");
  expect(source).toContain("时间错");
  expect(source).toContain("暂不确定");
});

test("S5-07 supplies a guided stability-check lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-07");

  expect(manifest.faqTermIds).toContain("fresh-problem-set");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-07"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-07-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("新题组");
  expect(source).toContain("交叉检查");
  expect(source).toContain("对照上一次");
  expect(source).toContain("稳定习惯");
});

test("S5-08 supplies a guided knowledge-map lesson and randomized choice quiz", async () => {
  const { source, manifest } = await expectImplementedInteractiveLesson("s5-08");

  expect(manifest.faqTermIds).toContain("knowledge-map");
  ...
  expect(source).toContain("建议掌握");
  expect(source).toContain("拓展");
  expect(source).toContain('courseId: "S5-08"');
  expect(source).toContain('type: "choice"');
  expect(source).toContain("不会在浏览器里运行 C++");
  expect(source).toContain("csp-cpp-s5-08-progress-v1");
  expect(source).toContain("course-plan / 自编");
  expect(source).toContain("题库暂时不可用");
  expect(source).toContain("知识地图");
  expect(source).toContain("薄弱模块");
  expect(source).toContain("赛前检查清单");
  expect(source).toContain("CSP-S 衔接条件");
});
...
test("the dashboard only exposes an interactive entry for implemented lessons", async () => {
  ...
  expect(dashboard).toContain('const implementedInteractiveLessons = ["S1-01", "S1-02", "S1-03", "S1-04", "S1-05", "S1-06", "S1-07", "S1-08", "S2-01", "S2-02", "S2-03", "S2-04", "S2-05", "S2-06", "S2-07", "S2-08", "S3-01", "S3-02", "S3-03", "S3-04", "S3-05", "S3-06", "S3-07", "S3-08", "S4-01", "S4-02", "S4-03", "S4-04", "S4-05", "S4-06", "S4-07", "S4-08", "S5-01", "S5-02", "S5-03", "S5-04", "S5-05", "S5-06", "S5-07", "S5-08"]');
